...
- **Breakpoint-gap fix (v1.3, 2026-06-26).** EPA's breakpoint tables have a deliberate 1-unit gap between ranges (O3 Good ends at 54 ppb and Moderate starts at 55, never 54.001). EPA's Reporting Technical Assistance Document specifies that the *measured* concentration must be **truncated** to the table's precision (integer ppb for O3 / PM10, 1 decimal µg/m³ for PM2.5) before lookup, so the truncated value always lands cleanly in one range. We weren't doing that, and on 2026-06-26 the O3 8-hr mean crossed 54 ppb and arrived at 54.07 — which fell in the 54-55 gap, dropped through every range, and the old fallback `return top[3], top[4]` returned the **last** breakpoint's max — AQI 300 from what was actually a Good-tier reading. Fixed by adding a `trunc_decimals` parameter to `aqi_from_concentration` and changing the fallback so a gap-fall-through returns `None` (drops the pollutant row) rather than the table max — better to publish nothing than to flash Very-Unhealthy from a Good day.
...
- ~~**Breakpoint-gap fall-through to AQI 300.**~~ **Resolved 2026-06-26.** EPA breakpoint tables have 1-unit gaps (O3 ends at 54 / starts at 55, etc.); the lookup must truncate the input to the table's precision per EPA's Reporting Technical Assistance Document. The old code didn't, and on 2026-06-26 a real 54.07 ppb O3 8-hr mean fell in the 54-55 gap, dropped through, and the fallback returned the table max — AQI 300 from a Good-tier reading. Fixed by truncating per pollutant (integer ppb for O3 / PM10, 1 decimal for PM2.5) and hardening the fallback to drop the pollutant row rather than emit the table max.
